<template>
  <div class="dashboard-container">
    ...
    <main class="main-content">
      ...
      <div class="projects-grid">
        ...
        <div v-for="project in projects" :key="project.idProyecto" class="project-card">
          <div class="project-header">
            <span class="project-number">{{ project.idProyecto }}</span>
            <div class="project-menu">
              <button class="project-menu-btn" @click.stop="toggleMenu(project.idProyecto)">
                <span class="dots">⋮</span>
              </button>
              <div v-if="activeMenu === project.idProyecto" class="menu-dropdown">
                <button class="menu-item delete" @click.stop="confirmDelete(project)">
                  Eliminar proyecto
                </button>
              </div>
            </div>
          </div>
          <div class="project-content" @click="handleProjectClick(project)">
  ...
  <!-- Modal de confirmación de acceso al proyecto -->
  <div v-if="showAccessModal" class="modal-overlay">
    <div class="modal-content">
      <h3>Acceder al Proyecto</h3>
      <p>¿Quiere ingresar al proyecto "{{ selectedProject?.nameProyect }}"?</p>
      <div class="modal-actions">
        <button class="btn-cancel" @click="showAccessModal = false">Cancelar</button>
        <button class="btn-confirm" @click="accessProject">Confirmar</button>
      </div>
    </div>
  </div>
</template>
...
const accessProject = () => {
  // Guardar el ID del proyecto en el store
  ProjectStore.projectId = selectedProject.value.idProyecto;
  // Cerrar el modal
  showAccessModal.value = false;
  // Navegar a la vista de áreas de gestión
