...
import gestion.proyectos.gestionproyectos.Service.ScopeAndTraceabilityMatrixService;
...
@RestController
@RequestMapping("/api/documents/TraceabilityMatrix")
@CrossOrigin("*")
public class ScopeAndTraceabilityMatrixController {

    private static final Logger logger = LoggerFactory.getLogger(AssumptionsDocumentController.class);

    @Autowired
    private ScopeAndTraceabilityMatrixService ScopeServcie;
    ...
    public ResponseEntity<?> generateAssumptionsDocument(
        ...
        try {
            ...
            byte[] pdfContent =  ScopeServcie.generateDocument1(requestData, idExit);
            String filename = generateFilename("alcance_y_matriz_de_trazabilidad");
